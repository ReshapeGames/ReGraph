...
namespace Reshape.ReGraph
{
    ...
    public class SaveBehaviourNode : BehaviourNode
    {
        ...
        protected override void OnStart (GraphExecution execution, int updateId)
        {
            ...
            {
                LogWarning("Found an empty Save Behaviour node in " + context.gameObject.name);
            }
            else if (executionType is ExecutionType.SaveVariables or ExecutionType.LoadVariables)
            {
                ...
                else if (executionType is ExecutionType.SaveVariables)
                {
                    if (saveType == SaveType.Append)
                    {
                        ...
                        if (!op.success)
                        {
                            LogWarning($"{saveFile} variables not success append save in {context.gameObject.name}.");
                        }
                        else
                        {
                            ...
                            if (!op.success)
                                LogWarning($"{saveFile} variables not success overwrite save in {context.gameObject.name}.");
                        }
                    }
                    else
                    {
                ...
                else if (executionType is ExecutionType.LoadVariables)
                {
                    ...
                    if (!op.success)
                    {
                        LogWarning($"{saveFile} variables not success load in {context.gameObject.name}.");
                    }
                    else
                    {
        ...
        public override string GetNodeViewDescription ()
        {
            ...
            {
                return $"Save {variables.Count} variables";
            }
            else if (executionType == ExecutionType.LoadVariables && !string.IsNullOrEmpty(saveFile) && variables != null && variables.Count > 0)
            {
                return $"Load {variables.Count} variables";
            }
